...
- **Cancellable** - Supports `CancellationToken`
- **Single permit** - Guarantees exclusivity
- **Fair** - FIFO ordering
...
Since late binding has no IntelliSense, you need other ways to discover available methods and properties.

### Method 1: Vendor Documentation

Check your simulator's:
- API Reference Manual
- Automation Guide
- SDK Documentation
- Sample scripts (VBA, Python, etc.)

### Method 2: Object Browser (Visual Studio)

If the COM type library is registered:

1. View → Object Browser (Ctrl+Alt+J)
2. Add Reference → COM → Select type library
3. Browse available types, methods, properties

## Lifecycle Management

Proper initialization and shutdown are critical for COM reliability.
    ...
    : base(logger, config.Automation)
{
    ...
    try
    {
        ...
        // Retrieve version once at startup
        _version = Server.Version;

        // Disconnect (will reconnect when needed)
        Shutdown();
    }
    finally
    {
