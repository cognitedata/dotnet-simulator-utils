                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    foreach (var routineRev in routineRevisions)
                    {
                        ...
                        if (scheduledJobs.TryGetValue(routineRev.RoutineExternalId, out var job))
                        {
                            ...
                            {
                                _logger.LogDebug("Cancelling job for RoutineRevision : {RoutineRevisionId} due to new configuration detected.", routineRev.ExternalId);
                        ...
                        if (!scheduledJobs.TryGetValue(routineRev.RoutineExternalId, out var existingJob))
                        {
                            try
                            {
                                ...
                                _logger.LogDebug("Created new job for schedule: {Schedule} with id {RevisionId}", routineRev.Configuration.Schedule.CronExpression, routineRev.ExternalId);
                                scheduledJobs.Add(routineRev.RoutineExternalId, newJob);
                            }
                            catch (Exception e)
                            {
                                _logger.LogError("Exception while scheduling job for RoutineRevision : {RoutineRevisionId} Error: {ErrorMessage}. Skipping.", routineRev.ExternalId, e.Message);
                            }
                        }
                    }
            ...
            while (!mainToken.IsCancellationRequested || !job.TokenSource.Token.IsCancellationRequested)
            {
                ...
                if (delay.TotalMilliseconds > 0)
                {
                    try
                    {
                        ...
                        _logger.LogDebug("Job executed at: {ExecutionTime} for routine revision: {RevisionId}", DateTime.UtcNow, routineRev.ExternalId);
                    }
                    catch (TaskCanceledException)
                    {
                        _logger.LogDebug("Job cancelled for routine revision: {RevisionId} breaking out of loop", routineRev.ExternalId);
                        break;
                    }
                    ...
                    if (!revisionExists)
                    {
                        _logger.LogDebug("Job not found for routine: {RoutineId} breaking out of loop", routineRev.RoutineExternalId);
                        break;
                    }
